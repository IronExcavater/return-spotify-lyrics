import { ColumnsIcon, GridIcon, RowsIcon } from '@radix-ui/react-icons';
...
import { ControlGroup, ControlSeparator } from './ControlGroup';

const INFINITE_LABEL = '\u221E';

type SectionControlGroupProps = {
...
export function TypeControlGroup({
    editor,
    editing,
    onChange,
    sectionId,
}: SectionControlGroupProps) {
    return (
        <ControlGroup
            id="type"
            label="Type"
                ...
                <Flex align="center" direction="row" wrap="nowrap" gap="2">
                    <StepperControl
                        ...
                        onValueChange={(value) =>
                            editor.changeCount(
                                value,
                                (next) => clampCount(next, editor.rowLimits),
                                editor.setRowsDraft,
                                'rows'
                            )
                        }
...
export function WidthControlGroup(props: SectionControlGroupProps) {
    const { editor, editing, onChange, sectionId } = props;
    if (editor.mode !== 'h-list') return null;
    ...
    return (
        <>
            ...
            >
                <StepperControl
                    label="Width"
                    value={editor.widthDraft ?? editor.displayWidthStr}
                        ...
                        onChange(sectionId, {
                            columnWidth:
                                parsed === undefined
                                    ...
                                    : editor.clampColumnWidth(parsed),
                        });
                    }}
                    onValueBlur={() => {
                        if (editor.widthDraft === null) return;
                        const parsed = editor.parseClamp(editor.widthDraft);
                        onChange(sectionId, {
                            columnWidth:
                    ...
                    suffix={
                        <Text size="1" color="gray" className="px-px">
                            px
                        </Text>
                    }
                    disabled={!editing}
...
export function ClampControlGroup(props: SectionControlGroupProps) {
    const { editor, editing, onChange, sectionId } = props;
    if (editor.mode !== 'v-list') return null;
    ...
    return (
        <>
            ...
            >
                <StepperControl
                    label="Clamp"
                    value={editor.clampDraft ?? editor.displayClampStr}
                    ...
                    onValueBlur={() => {
                        ...
                        const parsed = editor.parseClamp(editor.clampDraft);
                        onChange(sectionId, {
                            rowHeight:
                    ...
                    onValueFocus={() =>
                        editor.setClampDraft(
                            editor.displayClampStr === INFINITE_LABEL
                                ? ''
...
export function CardControlGroup(props: SectionControlGroupProps) {
    const { editor, editing, onChange, sectionId } = props;
    if (editor.mode !== 'card') return null;
    ...
    return (
        <>
            ...
            >
                <StepperControl
                    label="Size"
                    value={editor.cardSizeDraft ?? editor.displayCardSizeStr}
                    placeholder="2"
                    onDecrement={() => {
                        editor.setCardSizeDraft(null);
                        onChange(sectionId, {
                            cardSize: Math.max(1, editor.cardSize - 1) as
                                | 1
                                | 2
                                | 3,
                        });
                    ...
                    onIncrement={() => {
                        editor.setCardSizeDraft(null);
                        onChange(sectionId, {
                            cardSize: Math.min(3, editor.cardSize + 1) as
                                | 1
                                | 2
                                | 3,
                        });
